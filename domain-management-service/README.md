...
Manages domains and WordPress integration

## Overview

The Domain Management Service is a microservice that handles manages domains and wordpress integration. It's part of an event-driven architecture using Flask, PostgreSQL, and RabbitMQ.
...
cd domain-management-service

# Start all services
docker-compose up -d

# View logs
docker-compose logs -f domain-management-service
...
export DB_NAME=domain_management_dev
...
- **WordPressSite**: [Description needed]
- **DomainRecord**: [Description needed]
...
docker build -t domain-management-service:latest --target production .
...
docker run -d \
  ...
  domain-management-service:latest
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: domain-management-service
        image: domain-management-service:latest
        ports:
        - containerPort: 5003
        livenessProbe:
          httpGet:
...
### Logging
- Logs are written to `logs/domain-management-service.log`
