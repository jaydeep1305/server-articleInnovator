...
The Ai Configuration Service is a microservice that handles manages ai model configurations and settings. It's part of an event-driven architecture using Flask, PostgreSQL, and RabbitMQ.
...
cd ai-configuration-service

# Start all services
docker-compose up -d

# View logs
docker-compose logs -f ai-configuration-service
...
export DB_NAME=ai_configuration_dev
...
- **AIModel**: [Description needed]
- **Configuration**: [Description needed]
- **ModelProvider**: [Description needed]
...
docker build -t ai-configuration-service:latest --target production .
...
docker run -d \
  ...
  ai-configuration-service:latest
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: ai-configuration-service
        image: ai-configuration-service:latest
        ports:
        - containerPort: 5004
        livenessProbe:
          httpGet:
...
### Logging
- Logs are written to `logs/ai-configuration-service.log`
