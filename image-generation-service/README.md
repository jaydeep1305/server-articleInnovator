...
The Image Generation Service is a microservice that handles handles ai image generation and processing. It's part of an event-driven architecture using Flask, PostgreSQL, and RabbitMQ.
...
export DB_NAME=image_generation_dev
...
- **ImageRequest**: [Description needed]
- **GeneratedImage**: [Description needed]
- **ImageTemplate**: [Description needed]
...
docker build -t image-generation-service:latest --target production .
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: image-generation-service
        image: image-generation-service:latest
        ports:
        - containerPort: 5005
        livenessProbe:
          httpGet:
...
### Logging
- Logs are written to `logs/image-generation-service.log`
