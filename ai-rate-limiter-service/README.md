...
The Ai Rate Limiter Service is a microservice that handles ai api rate limiting and quota management. It's part of an event-driven architecture using Flask, PostgreSQL, and RabbitMQ.
...
cd ai-rate-limiter-service

# Start all services
docker-compose up -d

# View logs
docker-compose logs -f ai-rate-limiter-service
...
export DB_NAME=ai_rate_limiter_dev
...
- **RateLimit**: [Description needed]
- **Usage**: [Description needed]
- **Quota**: [Description needed]
...
docker build -t ai-rate-limiter-service:latest --target production .

# Run production container
docker run -d \
  --name ai-rate-limiter-service \
  -p 5011:5011 \
  -e DB_HOST=your-db-host \
  -e DB_USER=your-db-user \
  -e DB_PASSWORD=your-db-password \
  -e DB_NAME=your-db-name \
  ai-rate-limiter-service:latest
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: ai-rate-limiter-service
        image: ai-rate-limiter-service:latest
        ports:
        - containerPort: 5011
        livenessProbe:
          httpGet:
...
### Logging
- Logs are written to `logs/ai-rate-limiter-service.log`
