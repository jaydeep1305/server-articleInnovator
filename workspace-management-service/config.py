"""
Configuration module for Workspace Management Service.

This module provides environment-specific configuration classes that handle
workspace management settings, database connections, event bus configuration,
and workspace-related business logic configurations.
...
class DatabaseConfig:
    """Database configuration with type safety for workspace data."""
    host: str
    port: int
    username: str
    password: str
...
class EventBusConfig:
    """Event bus configuration for workspace events."""
    host: str
    port: int
    username: str
    password: str
    virtual_host: str = "/"
    exchange_name: str = "workspace_events"
...
class BaseConfig:
    """Base configuration class for Workspace Management Service."""
    
    # Flask Settings
    SECRET_KEY: str = os.environ.get('SECRET_KEY', 'dev-workspace-secret-key')
    DEBUG: bool = False
    TESTING: bool = False
    ...
    SERVICE_NAME: str = "workspace-management-service"
    SERVICE_VERSION: str = "1.0.0"
    
    # Workspace Business Rules
    MAX_WORKSPACES_PER_USER: int = int(os.environ.get('MAX_WORKSPACES_PER_USER', '10'))
    MAX_MEMBERS_PER_WORKSPACE: int = int(os.environ.get('MAX_MEMBERS_PER_WORKSPACE', '100'))
    WORKSPACE_NAME_MIN_LENGTH: int = 3
    WORKSPACE_NAME_MAX_LENGTH: int = 50
    WORKSPACE_DESCRIPTION_MAX_LENGTH: int = 500
    
    # File Storage Configuration
    WORKSPACE_STORAGE_PATH: str = os.environ.get('WORKSPACE_STORAGE_PATH', '/app/storage/workspaces')
    MAX_WORKSPACE_STORAGE_GB: int = int(os.environ.get('MAX_WORKSPACE_STORAGE_GB', '5'))
    
    # Event Configuration
    WORKSPACE_EVENTS_ENABLED: bool = os.environ.get('WORKSPACE_EVENTS_ENABLED', 'true').lower() == 'true'
    EVENT_RETRY_ATTEMPTS: int = int(os.environ.get('EVENT_RETRY_ATTEMPTS', '3'))
    
    # Rate Limiting
    RATE_LIMIT_WORKSPACE_CREATION: str = "5 per hour"
    RATE_LIMIT_MEMBER_INVITE: str = "20 per hour"
    ...
    def get_event_bus_config(cls) -> EventBusConfig:
        ...
        return EventBusConfig(
            ...
            exchange_name=os.environ.get('RABBITMQ_EXCHANGE', 'workspace_events')
        )
...
class DevelopmentConfig(BaseConfig):
    ...
    RATE_LIMIT_WORKSPACE_CREATION: str = "10 per hour"
    ...
    def get_database_config(cls) -> DatabaseConfig:
        ...
        return DatabaseConfig(
            ...
            database=os.environ.get('DB_NAME', 'workspace_management_dev'),
            pool_size=5,
            max_overflow=10,
            pool_timeout=20
        )
...
class TestingConfig(BaseConfig):
    ...
    WORKSPACE_EVENTS_ENABLED: bool = False
...
class ProductionConfig(BaseConfig):
    ...
    RATE_LIMIT_WORKSPACE_CREATION: str = "3 per hour"
...
def setup_logging(config: BaseConfig) -> None:
    """Setup logging configuration for workspace service."""
    import logging
